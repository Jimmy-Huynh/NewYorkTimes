package com.tvnsoftware.newyorktimes.Utils;

/**
 * Created by TamHH on 6/26/2017.
 */

public class Contant {
    public static final String BASE_URL = "https://api.nytimes.com/svc/search/v2/";
    public static final String API_KEY = "c42ba6d82eff4a30bb66b26ba0c8201f";
    public static final String BASE_URL_IMAGES = "http://www.nytimes.com/";
    public static final String DOC_WEB = "doc_object";

}
